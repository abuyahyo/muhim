...
(function () {
  ...
  // Соддагина SVG иллюстратция — ҳар жойга мос геометрия.
  function iconFor(id) {
    if (id === 'haram') {
      return '<svg viewBox="0 0 120 120" aria-hidden="true">'
           + '<rect x="35" y="40" width="50" height="55" rx="2" fill="rgba(0,0,0,0.6)" stroke="rgba(255,255,255,0.35)" stroke-width="2"/>'
           + '<rect x="35" y="58" width="50" height="6" fill="rgba(251,191,36,0.8)"/>'
           + '<path d="M 30 100 L 90 100" stroke="rgba(255,255,255,0.4)" stroke-width="2"/>'
           + '</svg>';
    }
    if (id === 'nabawi') {
      return '<svg viewBox="0 0 120 120" aria-hidden="true">'
           + '<rect x="20" y="55" width="80" height="45" fill="rgba(255,255,255,0.18)"/>'
           + '<path d="M 40 55 Q 60 25 80 55 Z" fill="rgba(34,197,94,0.85)"/>'
           + '<line x1="60" y1="22" x2="60" y2="12" stroke="rgba(255,255,255,0.7)" stroke-width="2"/>'
           + '<circle cx="60" cy="10" r="3" fill="rgba(255,255,255,0.8)"/>'
           + '<path d="M 20 100 L 100 100" stroke="rgba(255,255,255,0.4)" stroke-width="2"/>'
           + '</svg>';
    }
    if (id === 'aqsa') {
      return '<svg viewBox="0 0 120 120" aria-hidden="true">'
           + '<rect x="20" y="60" width="80" height="40" fill="rgba(255,255,255,0.18)"/>'
           + '<path d="M 35 60 Q 60 25 85 60 Z" fill="rgba(251,191,36,0.9)"/>'
           + '<line x1="60" y1="22" x2="60" y2="12" stroke="rgba(255,255,255,0.7)" stroke-width="2"/>'
           + '<circle cx="60" cy="10" r="3" fill="rgba(251,191,36,0.95)"/>'
           + '<path d="M 20 100 L 100 100" stroke="rgba(255,255,255,0.4)" stroke-width="2"/>'
           + '</svg>';
    }
    return '';
